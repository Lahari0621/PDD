...
const FALLACY_PATTERNS = [
  ...
  },
  {
    ...
    description: 'Manipulating emotions rather than using logical reasoning.',
    severity: 'medium',
    color: '#EC4899',
  },
  {
...
class FallacyDetector {
        ...
        if (match) {
          ...
          if (!detected.find(d => d.type === fallacy.type)) {
            detected.push({
              ...
              confidence: 0.75 + Math.random() * 0.2, // 75-95% for rule-based
  ...
  async detect(text) {
    try {
      ...
      const [ruleBasedResults, hfResults] = await Promise.all([
        Promise.resolve(this.detectWithRules(text)),
